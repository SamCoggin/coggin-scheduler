...
// THE PARTS: a sub-contractor clearance with a yard unload becomes a contractor job plus a yard leg on the due day.
setTimeout(()=>{ const all=global.window.__allJobs(); const main=all.find(j=>j.id==='c4'), leg=all.find(j=>j.isLeg&&j.leg.type==='yard'&&j.leg.parent==='c4');
  console.log('job parts:', !!main&&main.contractor==='Example Haulage'&&!!leg&&leg.date==='2026-09-16'&&!leg.site&&leg.movement==='Yard unload');
},800);
